...
export default function SocialMediaManagerPage() {
  ...
  }, [status, router, fetchScheduledPosts]);
  ...
  const handleSchedulePost = async () => {
      ...
      try {
          const res = await fetch('/api/social/schedule', {
              ...
              body: JSON.stringify({ platform: selectedPlatform, content: postContent, hashtags, scheduledAt: new Date(`${scheduleDate}T${scheduleTime}`).toISOString() })
          });
          if (!res.ok) throw new Error((await res.json()).message);
          setPostContent(''); setHashtags([]); setIsScheduleModalOpen(false);
          await fetchScheduledPosts();
      } catch (err) { setError(err.message); } finally { setIsScheduling(false); }
  };
  ...
  const handlePostNow = async () => {
    ...
    try {
        const res = await fetch('/api/social/post', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ platform: selectedPlatform, content: postContent })
        });
        if (!res.ok) throw new Error((await res.json()).message);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md border">
            ...
            <div className="mt-4 flex justify-between items-center">
                ...
                <div className="flex items-center gap-x-4">
                    <button onClick={handlePostNow} disabled={postContent.length === 0 || isOverLimit || isPosting} className="flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none disabled:bg-gray-400">
                        {isPosting ? 'Posting...' : 'Post Now'}
                    </button>
                    <button onClick={() => setIsScheduleModalOpen(true)} disabled={postContent.length === 0 || isOverLimit} className="flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none disabled:bg-gray-400">
        ...
        <div className="lg:col-span-1 space-y-8">
            <div className="bg-white p-6 rounded-lg shadow-md border space-y-4">
                ...
                <button onClick={handleGeneratePost} disabled={isGenerating} className="w-full flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none disabled:bg-gray-400">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4">
                ...
                <div className="flex justify-end space-x-3">
                    <button type="button" onClick={() => setIsScheduleModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Cancel</button>
