...
const ComposerTabContent = ({ onPostScheduled, scheduledPosts, postContent, setPostContent, selectedPlatform, setSelectedPlatform }) => {
    ...
    useEffect(() => {
        // FIX: The userImages state is not being set here, so the fetch is removed.
        // fetch('/api/images').then(res => res.json()).then(setUserImages).finally(() => setIsLoadingImages(false));
        setIsLoadingImages(false);
    }, []);

    // FIX: Removed unused setNodeRef variable
    const { setNodeRef: dropRef } = useDroppable({ id: 'post-composition-area' });
    const handleDragStart = (event) => setActiveDragId(event.active.id);
    const handleDragEnd = () => { /* Logic removed as per previous fixes */ };
        ...
        <>
            <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} collisionDetection={pointerWithin}>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-gap-8" ref={dropRef} style={{ minHeight: '250px' }}>
                ...
                <ImageManager onImageAdd={handleImageAdded} isLoading={isLoadingImages} />
